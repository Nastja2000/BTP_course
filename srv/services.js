...
class ProcessorService extends cds.ApplicationService {
  ...
  async init() {
    ...
    this.northwind = await cds.connect.to('northwind_btp');
    ...
    this.on('getOrdersBTP', (req) => this.onGetOrdersBTP(req));

    return super.init();
  }
  
  async onGetOrdersBTP(req) {
    try {
      const result = await this.northwind.get('/Orders');
      // run(SELECT.from('Orders').limit(10));
      return result;
    } catch (error) {
      console.error('### ERRPOR Northwind:', {
